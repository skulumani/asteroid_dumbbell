...
def plot_inertial(time, state, ast, dum, pgf_save=False, fwidth=0.5):
    """Plot the state in the inertial frame

    Generate a bunch of useful plots of the inertial state trajectory
...
def plot_controlled_blender_inertial_fixed_asteroid(time, state, ast, dum, pgf_save, fwidth,
                                     desired_translation_func, desired_attitude_func):
    """Plot the state and desired command for the controlled system generated using Blender Sim
    ...
    for ii, t in enumerate(time):
        x_des[ii,:], xd_des[ii, :], xdd_des[ii, :] = desired_translation_func(t, 3600, 1)
        Rd, Rd_dot, wd, wd_dot = desired_attitude_func(t, state[ii, :])
...
def plot_controlled_blender_asteroid(time, state, ast, dum, pgf_save, fwidth,
    ...
    for ii, t in enumerate(time):
        ...
        Rd, Rd_dot, wd, wd_dot = desired_attitude_func(t, state[ii, :], ast)
    ...
    # three dimensional plot of the trajectory
    frame_indicies = np.linspace(time[0], time[-1], 100, dtype=int)
