...
void AttitudeController::random_sweep_attitude(std::shared_ptr<const State> state_in) {
    // extract out the elements of the state
    Eigen::Vector3d pos, vel, ang_vel;
    Eigen::Matrix<double, 3, 3> R;
    pos << state_in->get_pos();
    ...
    R << state_in->get_att();
    ang_vel << state_in->get_ang_vel();
    double t = state_in->get_time();

    // desired attitude such that b1 points to origin/asteroid
    Eigen::Matrix<double, 3, 1> b1_des(3), b2_des(3), b3_des(3), z_axis(0, 0, 1);
    b1_des = (- pos + 0.1 * (Eigen::Vector3d() << std::sin(t), std::cos(t), std::sin(t)).finished()).normalized();
    b3_des = z_axis - (z_axis.dot(b1_des) * b1_des) ;
    b3_des = b3_des.normalized();
    b2_des = b3_des.cross(b1_des);
...
void Controller::refinement(const double& t,
    ...
    /* /1* Eigen::Vector3d desired_inertial_pos = inertial_landing_site.normalized() * (2.0 * desired_landing_site.norm() ); *1/ */
    /* /1* Eigen::Vector3d desired_inertial_pos = Ra * desired_landing_site.normalized() * (3.0 * desired_landing_site.norm()); *1/ */
