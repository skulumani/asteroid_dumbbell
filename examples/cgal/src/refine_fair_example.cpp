...
// extract vertices which are at most k (inclusive) far from vertex v in the graph
// of edges
...
int main(int argc, char* argv[]) {
    ...
    if (!input || !(input >> poly) || poly.empty() || !CGAL::is_triangle_mesh(poly)) {
        ...
        std::cout << "Valid input file. Created poly" << std::endl;
    }
    ...
    std::ofstream refined_off("data/refined.off");
    refined_off << poly;
    refined_off.close();
    std::cout << "Refinement added " << new_vertices.size() << " vertices." << std::endl;

    Polyhedron::Vertex_iterator v = poly.vertices_begin();
    std::advance(v, 82); // go to 82 vertex
    std::vector<Vertex_handle> region;
    extract_k_ring(v, 2, region); // get nearest 12-ring vertices
